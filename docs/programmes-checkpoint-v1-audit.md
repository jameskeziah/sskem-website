...
10. **Accessibility and responsive QA - private shell built:** automated keyboard, semantic/contrast, reduced-motion, no-JavaScript, mobile and 200%-zoom-equivalent checks are wired. Repeat them with the final adapter-fed approved fixture and complete manual NVDA/VoiceOver acceptance.
11. **Programme performance budget - built:** every build audits the 250 KB hero, system-font and 700 ms motion limits; the private browser gate enforces a 1.5 MB/32-request route cap, zero initial video/font transfer, no autoplay/infinite motion, LCP at or below 2.5 seconds and CLS at or below 0.1. Re-run with each exact approved media derivative.
12. **Expiry and rollback controls - built:** fees, schedule, results and admissions now carry approval-derived end dates; stale values are removed from the render projection, canonical safe fallbacks remain, receipts bind the exact transition and rollback planning refuses expired targets. Wire request-time/cache expiry evaluation and controlled receipt storage only with the future approved route resolver.
13. **Site-wide finishing - built:** the shared shell now includes generic noindex 404 metadata for dormant Programme routes, loading/error recovery, two skip targets, forced-colour focus treatment, complete print rules, central footer navigation and safer mobile drawer behavior. Browser/device acceptance remains before cutover.
14. **Management acceptance and activation:** only after real approved data, current evidence and exact receipts exist; then perform a separately authorised deployment/cutover.
