...
| Homepage arrival heading | 4 | Initial page entry below `64rem` | `opacity: 0`, bounded responsive `y` | Fully visible at authored position | `slow`, `emphasised` | Responsive heading token; three lines maximum | Once | Live heading reveal | Static final state | GSAP |
| Homepage desktop artwork | 4 | Public and private-review renders at `64rem` and above | Exact authored `/og.png` composition | Static, uncropped and unoverlaid artwork | None | None | Never | Same authored composition | Static final state | Responsive CSS |
| Homepage private desktop reveal | 4 | First private-review homepage entry in a tab | Fixed SSKEMS loading layer over the authored poster | Loading layer clips away to expose the static poster | `standard` through `ceremonial`, `exit` and `move` | None | Once per tab session when storage is available | Same bounded loading layer over the responsive hero | Never displayed | GSAP |
...
On desktop, the private loading layer reveals the same static authored poster
as the public render. Below `64rem`, the live hero copy and campus image retain
their responsive arrival. Institutional facts, programme availability and
admissions status never depend on animation.
