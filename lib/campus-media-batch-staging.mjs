...
export async function executeCampusMediaBatchStaging({
  ...
  try {
    ...
    if (currentFingerprint) {
      await renameWithRetry(target, backupTarget);
      backupCreated = true;
    }
    try {
      await renameWithRetry(temporaryTarget, target);
      committed = true;
    } catch (error) {
      if (backupCreated) await renameWithRetry(backupTarget, target);
      backupCreated = false;
      throw error;
    }
    if (backupCreated) {
    ...
    if (backupCreated && !(await pathState(target))) await renameWithRetry(backupTarget, target);
    throw error;
  }
}
