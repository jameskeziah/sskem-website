...
export async function createCampusMediaPublicationBatchPlan({
  ...
  publicRootState: suppliedPublicRootState,
  ...
  const publicRootState = suppliedPublicRootState ?? await targetState(publicRoot);
  ...
  if (publicRootState !== "absent") blockers.push("The production campus media root is not absent; atomic first publication refuses replacement.");
  ...
  const sourceDigest = stagedAudit.status === "verified"
    ? sha256([
      stagedAudit.batchSha256,
      sha256(registryText),
      ...stagedAudit.proposals.map((proposal) => `${proposal.recordId}:${proposal.sourceSha256}`),
    ].join("\n"))
    : null;
