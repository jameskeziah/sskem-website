import approvalManifest from "../content/approval-manifest.json" with { type: "json" };
...
export function isOpaqueEvidenceReference(value) {
  return typeof value === "string"
    && evidenceReferencePattern.test(value)
    && !privateEvidenceLocationPattern.test(value);
}
...
export async function loadApprovalManifest() {
  return structuredClone(approvalManifest);
}
  ...
  for (const [index, record] of manifest.records.entries()) {
    ...
    if (!Array.isArray(record.evidenceReferences)) {
      ...
      for (const reference of record.evidenceReferences) {
        if (!isOpaqueEvidenceReference(reference)) {
