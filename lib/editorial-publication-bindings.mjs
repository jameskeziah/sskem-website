...
export function validateEditorialPublicationBindings(registry, manifest) {
  ...
  for (const [index, binding] of registry.bindings.entries()) {
    ...
    reportUnknownKeys(binding, new Set(["bindingId", "approvalRecordIds", "contentType", "documentId", "revision", "contentDigestSha256", "boundOn", "ownerRole", "notes"]), path, add);
    ...
    if (!Array.isArray(binding.approvalRecordIds) || !binding.approvalRecordIds.length) {
      add("approval-record-ids", `${path}.approvalRecordIds`, "CMS bindings require at least one claim approval record.");
    } else {
      const localIds = new Set();
      for (const [approvalIndex, approvalRecordId] of binding.approvalRecordIds.entries()) {
        const approvalPath = `${path}.approvalRecordIds[${approvalIndex}]`;
        if (typeof approvalRecordId !== "string" || !approvalRecordIdPattern.test(approvalRecordId)) {
          add("approval-record-id", approvalPath, "CMS bindings must reference claim approval records.");
          continue;
        }
        if (localIds.has(approvalRecordId)) add("duplicate-binding-approval", approvalPath, "Approval IDs within one binding must be unique.");
        localIds.add(approvalRecordId);
        if (seenApprovalIds.has(approvalRecordId)) add("duplicate-approval-record", approvalPath, "One approval record may bind only one exact CMS revision.");
        seenApprovalIds.add(approvalRecordId);

        const approvalRecord = manifestRecords.get(approvalRecordId);
        if (!approvalRecord) add("missing-approval-record", approvalPath, "The referenced approval record does not exist in the canonical manifest.");
        else if (approvalRecord.kind !== "claim") add("approval-kind", approvalPath, "CMS editorial content must use claim approval records.");
        else if (approvalRecord.decision !== "approved") add("approval-decision", approvalPath, "A revision binding may be recorded only after every canonical claim is approved.");
      }
    }
